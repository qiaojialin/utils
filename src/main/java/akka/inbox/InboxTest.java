...
public class InboxTest extends UntypedAbstractActor {
    ...
    public static void main(String [] args){
        ...
        ActorRef inboxTest1 = system.actorOf(Props.create(InboxTest.class), "InboxTest1");

        Inbox inbox = Inbox.create(system);
        inbox.watch(inboxTest1);//监听一个actor

        //通过inbox来发送消息
        inbox.send(inboxTest1, Msg.WORKING);
        inbox.send(inboxTest1, Msg.DONE);
        inbox.send(inboxTest1, Msg.CLOSE);
